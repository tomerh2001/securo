...
export function TokenConnectDialog({
  ...
  const isHapoalim = provider === 'investment_feed' && token.trim().startsWith('hapoalim.')
  const i18nKey = `accounts.tokenConnect.${isHapoalim ? 'hapoalim' : isBestInvest ? 'best_invest' : provider}`

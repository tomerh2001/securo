...
describe('InvestmentAccountDetailPage', () => {
  ...
  it('preserves unknown valuation dates and uses a generic connection notice', async () => {
    ...
    expect(notice).toHaveTextContent('Connection needs attention. Showing the last available balance.')
    expect(within(notice).getByRole('link', { name: 'Connection details' })).toHaveAttribute('href', '/connections/connection-one')
